...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):  # i guess this is __getitem__, 'seg' is 'target'
        ...
        for j, i in enumerate(selected_keys):
            ...
            # case_all_data is the input, shape [2, D, H, W] - not cropped
        ...
        data = np.vstack(data)
        seg = np.vstack(seg)
        # data: batch_size, 1, D, H, W; seg: batch_size, 1, D, H, W
...
class DataLoader3D_orisize(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None):
        """
        ...
        :param oversample_foreground: half the batch will be forced to contain at least some foreground (equal prob for each of the foreground classes)
        """
        super(DataLoader3D_orisize, self).__init__(data, batch_size, None)
        if pad_kwargs_data is None:
            pad_kwargs_data = OrderedDict()
        self.pad_kwargs_data = pad_kwargs_data
        self.pad_mode = pad_mode
    ...
    def generate_train_batch(self):  # i guess this is __getitem__, 'seg' is 'target'
        ...
        for j, i in enumerate(selected_keys):
            ...
            crop_size = [-valid_bbox_x_lb + valid_bbox_x_ub, -valid_bbox_y_lb + valid_bbox_y_ub,
                         -valid_bbox_z_lb + valid_bbox_z_ub]
            case_all_data = self.resize_data_seg(case_all_data, crop_size)

            case_all_data_donly = case_all_data[:-1]
            case_all_data_segonly = case_all_data[-1:]
    ...
    def resize_data_seg(self, x, target_size):
        x_val = x[0]
        x_seg = x[1]
        D, H, W = x.shape[-3:]

        x_ = np.linspace(-1, 1, D)
        y_ = np.linspace(-1, 1, H)
        z_ = np.linspace(-1, 1, W)
        grid_input = (x_, y_, z_)
        volume_interp_fn_intensity = RegularGridInterpolator(grid_input, x_val,method="linear", bounds_error=False,fill_value=0)
        volume_interp_fn_label = RegularGridInterpolator(grid_input, x_seg,method="nearest", bounds_error=False, fill_value=0)

        x_ = np.linspace(-1, 1, target_size[0])
        y_ = np.linspace(-1, 1, target_size[1])
        z_ = np.linspace(-1, 1, target_size[2])
        px, py, pz = np.meshgrid(x_, y_, z_, indexing='ij')
        physical_pts = np.stack([px, py, pz], -1)  # [D, H, W, 3]
        mri_batch_val = volume_interp_fn_intensity(physical_pts)
        mri_batch_label = volume_interp_fn_label(physical_pts)
        y = np.concatenate([mri_batch_val[None, ...], mri_batch_label[None, ...]], axis=0)
        # print(y.shape)
        return y


if __name__ == "__main__":
    t = "Task1074_finegrained_bone_real_90"
    preprocessing_output_dir = "/p300/liyuwei/DATA_mri/Hand_MRI_segdata/nnUNet_preprocessed"
